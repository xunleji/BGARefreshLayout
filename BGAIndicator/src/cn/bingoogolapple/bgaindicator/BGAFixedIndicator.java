...
public class BGAFixedIndicator extends LinearLayout implements
		View.OnClickListener, View.OnFocusChangeListener, OnPageChangeListener {
	...
	public void initAttr(int attr, TypedArray typedArray) {
		if (attr == R.styleable.BGAIndicator_indicator_triangleColor) {
			 ...
			 */
			mTriangleHorizontalMargin = typedArray.getDimensionPixelSize(attr,
					mTriangleHorizontalMargin);
		} else if (attr == R.styleable.BGAIndicator_indicator_triangleHeight) {
			mTriangleHeight = typedArray.getDimensionPixelSize(attr,
					mTriangleHeight);
			...
			mTextSizeNormal = typedArray.getDimensionPixelSize(attr,
					mTextSizeNormal);
		} else if (attr == R.styleable.BGAIndicator_indicator_textSizeSelected) {
			mTextSizeSelected = typedArray.getDimensionPixelSize(attr,
					mTextSizeSelected);
			...
			mDividerWidth = typedArray.getDimensionPixelSize(attr,
					mDividerWidth);
		} else if (attr == R.styleable.BGAIndicator_indicator_dividerVerticalMargin) {
			mDividerVerticalMargin = typedArray.getDimensionPixelSize(attr,
					mDividerVerticalMargin);
		}
	}
	...
	private void initTab(int currentTab) {
		for (int index = 0; index < mTabCount; index++) {
			View tabIndicator = mInflater.inflate(R.layout.view_indicator,
					this, false);
			tabIndicator.setId(BSSEEID + index);
			tabIndicator.setOnClickListener(this);

			TextView titleTv = (TextView) tabIndicator
					.findViewById(R.id.tv_indicator_title);
			if (mTextColor != null) {
			...
			LayoutParams tabLp = new LayoutParams(0, LayoutParams.MATCH_PARENT,
					1);
			...
			if (index != mTabCount - 1 && mHasDivider) {
				LayoutParams dividerLp = new LayoutParams(mDividerWidth,
						LayoutParams.MATCH_PARENT);
				dividerLp.setMargins(0, mDividerVerticalMargin, 0,
						mDividerVerticalMargin);
	...
	private void resetTab(View tab, boolean isSelected) {
		...
		tv.setTextSize(TypedValue.COMPLEX_UNIT_PX,
				isSelected ? mTextSizeSelected : mTextSizeNormal);
