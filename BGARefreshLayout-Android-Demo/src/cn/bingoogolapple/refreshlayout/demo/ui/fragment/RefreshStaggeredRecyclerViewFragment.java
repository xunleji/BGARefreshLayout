 ...
 */
public class RefreshStaggeredRecyclerViewFragment extends BaseFragment
		implements BGARefreshLayout.BGARefreshLayoutDelegate,
		BGAOnRVItemClickListener, BGAOnRVItemLongClickListener {
	private static final String TAG = RefreshStaggeredRecyclerViewFragment.class
			.getSimpleName();
	...
	protected void processLogic(Bundle savedInstanceState) {
		mRefreshLayout.setCustomHeaderView(
				DataEngine.getCustomHeaderView(mApp), true);
	...
	protected void onUserVisible() {
		mNewPageNumber = 0;
		mMorePageNumber = 0;
		mEngine.loadDefaultStaggeredData().enqueue(
				new Callback<List<StaggeredModel>>() {
					@Override
					public void onResponse(
							Response<List<StaggeredModel>> response) {
	...
	public void onBGARefreshLayoutBeginRefreshing(BGARefreshLayout refreshLayout) {
		...
		mEngine.loadNewStaggeredData(mNewPageNumber).enqueue(
				new Callback<List<StaggeredModel>>() {
					@Override
					public void onResponse(
							final Response<List<StaggeredModel>> response) {
	...
	public boolean onBGARefreshLayoutBeginLoadingMore(
		...
		mEngine.loadMoreStaggeredData(mMorePageNumber).enqueue(
				new Callback<List<StaggeredModel>>() {
					@Override
					public void onResponse(
							final Response<List<StaggeredModel>> response) {
